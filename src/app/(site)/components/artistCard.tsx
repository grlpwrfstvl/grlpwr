...
const ArtistCard: React.FC<ArtistCardProps> = ({ artist, size = 300, index }) => {
  ...
  return (
    <Link href={`/artists/${artist.slug}`} key={artist._id} className={`-mx-6 -my-6 z-10 h-80 w-max relative overflow-visible ${isMiddleCard ? 'mb-0' : 'mt-0'} transition-transform transform hover:-translate-y-2`}>
      ...
      <h2 className="z-20 transform absolute p-4 bottom-1/4 left-1/4 transform font-extrabold text-2xl text-white drop-shadow-lg">{artist.name}</h2>
    </Link>
  );
};
